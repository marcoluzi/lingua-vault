<div x-data="{ open: false }">
    @if (isset($label))
        <label class="block text-sm font-medium leading-6 {{ isset($lightLabel) ? 'text-white' : '' }}">
            {{ $label }}
        </label>
    @endif
        ...
        <ul class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
            ...
            @foreach ($items as $item)
                <li class="text-gray-900 relative cursor-default select-none py-2 pl-3 pr-9"
                    :class="{
                        'bg-indigo-600 text-white': '{{ $selectedItem['value'] }}'
                        === '{{ $item['value'] }}',
                        'text-gray-900': '{{ $selectedItem['value'] }}'
                        !== '{{ $item['value'] }}'
                    }"
                    role="option"
                    @click="
                        ...
                        @elseif(isset($item['link']))
                            window.location.href = '{{ $item['link'] }}' @endif
                        open = false;
                    ">
                    <div class="flex items-center">
                        ...
                        <span class="font-normal ml-3 block truncate"
                            :class="{
                                'font-semibold': '{{ $selectedItem['value'] }}'
                                === '{{ $item['value'] }}'
                            }">
                            {{ $item['label'] }}
                        </span>
