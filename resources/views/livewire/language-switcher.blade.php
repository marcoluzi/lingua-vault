    ...
    <div class="relative mt-2">
        <button type="button"
            ...
            <span class="flex items-center">
                ...
                <span class="ml-3 block truncate">{{ $selectedLanguageName ?? __('Select language') }}
                </span>
            </span>
        ...
        <ul x-show="open" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
            ...
            @foreach ($languages as $language)
                <li class="text-gray-900 relative cursor-default select-none py-2 pl-3 pr-9"
                    ...
                    :class="{
                        'bg-indigo-600 text-white': activeIndex === {{ $loop->index }},
                        'text-gray-900': !(
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="font-normal ml-3 block truncate"
                            :class="{
                                'font-semibold': activeIndex === {{ $loop->index }},
                                'font-normal': !(
                                    activeIndex === {{ $loop->index }})
                            }">{{ $language->name }}</span>
                    </div>
